...
def match_flash_action_group(token_flow_list):
    ...
    for borrow_ptr in range(length - 1, -1, -1):
        ...
        borrow_symbol = borrow_row[3]
        borrow_action_type = borrow_row[5]
        borrow_swap_pool = borrow_row[6]
        borrower = borrow_row[2]
        ...
        for repay_ptr in range(length):
            ...
            repay_symbol = repay_row[3]
            repay_action_type = repay_row[5]
            repay_swap_pool = repay_row[6]
            repayer = repay_row[1]
    ...
    while len(borrow_index_stake) > 0:
        ...
        # edit group_type
        token_flow_list[borrow_ptr][7] = re.sub(
            r":[0-9]+",
            ":flash_borrow:%d" % (flash_action_pair_cout),
            token_flow_list[borrow_ptr][7],
        )
        token_flow_list[borrow_ptr] = token_flow_list[borrow_ptr] + [
            flash_action_pair_cout
        ]
        token_flow_list[repay_ptr][7] = re.sub(
            r":[0-9]+",
            ":flash_repay:%d" % (flash_action_pair_cout),
            token_flow_list[repay_ptr][7],
        )
        token_flow_list[repay_ptr] = token_flow_list[repay_ptr] + [
            flash_action_pair_cout
        ]
